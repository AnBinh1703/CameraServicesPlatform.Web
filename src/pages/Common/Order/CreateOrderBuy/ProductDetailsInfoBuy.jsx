...
import { Card, Col, Descriptions, Row, Spin, Form, InputNumber, message } from "antd";
...
const ProductDetailsInfoBuy = ({ product, loading }) => {
  ...
  const validateQuantity = (_, value) => {
    if (!value) {
      return Promise.reject('Vui lòng nhập số lượng đặt hàng');
    }
    if (value > product?.quantity) {
      return Promise.reject(`Số lượng đặt hàng không thể vượt quá ${product?.quantity} sản phẩm`);
    }
  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        rules={[
          { required: true, message: "Vui lòng nhập số lượng đặt hàng" },
      ...
      >
        <InputNumber 
          ...
          onChange={(value) => {
            if (value > product?.quantity) {
              message.error(`Số lượng đặt hàng không thể vượt quá ${product?.quantity} sản phẩm`);
            }
          }}
          style={{ width: '100%' }}
        />
