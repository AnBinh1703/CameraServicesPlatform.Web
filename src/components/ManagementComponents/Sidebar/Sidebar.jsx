...
const SideBar = () => {
  ...
  const menuItems = {
    STAFF: [
      ...
      { name: "GIAO DỊCH", link: "manage-transaction-system", icon: <RiFileListLine /> },
      { name: "QUẢN LÍ GÓI ĐĂNG KÍ", link: "manage-combo", icon: <RiSettings4Line /> },
    ],

    SUPPLIER: [
      { name: "THỐNG KÊ", link: "dashboard", icon: <RiDashboardLine /> },
      { name: "ĐĂNG KÍ GÓI", link: "personal-supplier", icon: <RiHome4Line /> },
      { name: "VOUCHER", link: "manage-voucher-of-supplier", icon: <RiGiftLine /> },
      { name: "QUẢN LÍ SẢN PHẨM", link: "manage-product-of-supplier", icon: <RiStore3Line /> },
      { name: "QUẢN LÍ ĐƠN HÀNG", link: "manage-order", icon: <RiShoppingBag3Line /> },
      { name: "QUẢN LÍ ĐIỀU KHOẢN HỢP ĐỒNG", link: "manage-contract-template", icon: <RiFileListLine /> },
      { name: "SẢN PHẨM BỊ BÁO CÁO", link: "manage-product-report-by-supplier", icon: <RiFileWarningLine /> },
      { name: "TRÁCH NHIỆM VÀ QUYỀN LỢI", link: "responsibilities-rights", icon: <RiHandCoinLine /> },
    ],

    ADMIN: [
      { name: "THỐNG KÊ", link: "dashboard", icon: <RiDashboardLine /> },
      { name: "NGƯỜI DÙNG", link: "manage-user", icon: <RiUser3Line /> },
      { name: "TỔNG QUẢN LÍ SẢN PHẨM HỆ THỐNG", link: "manage-product", icon: <RiStore3Line /> },
      { name: "QUẢN LÍ GÓI", link: "manage-combo", icon: <RiFileListLine /> },
      { name: "QUẢN LÍ CHÍNH SÁCH", link: "manage-policy", icon: <RiSettings4Line /> },
  ...
  const renderMenu = (items) => {
    ...
    return items.map((item) => (
      <li key={item.link} className="my-1">
        ...
        >
          {item.icon && (
            <span className={`w-6 h-6 transition-transform duration-200 ${
              location.pathname.includes(item.link) ? "scale-110" : ""
            }`}>
              {item.icon}
            </span>
          )}
          <span className={`ml-3 font-medium ${!isOpen && "hidden"} whitespace-nowrap`}>
            {item.name}
          </span>
        </NavLink>
      </li>
  ...
  return (
    <div className="flex">
      ...
      >
        ...
        <nav className="p-3">
          <ul className="space-y-1">{roleName && renderMenu(menuItems[roleName])}</ul>
        </nav>
      </div>
